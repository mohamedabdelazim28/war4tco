-- Seed demo mechanics for the booking area.
-- Creates 3 auth users with role mechanic, then mechanics rows.
-- Password for all seed mechanics: MechanicPass123!
...
BEGIN
  -- Get instance_id: from auth.instances first, then from any existing auth user
  ...
  IF inst_id IS NOT NULL THEN
    -- Insert auth users (trigger creates profiles with role from raw_user_meta_data)
    ...
    ON CONFLICT (user_id) DO UPDATE SET
      ...
      updated_at = now();
  ELSE
    -- No instance_id: add mechanics rows for any existing mechanic profiles (so you can create users in Dashboard first)
    INSERT INTO public.mechanics (user_id, workshop_name, experience_years, rating, availability_status)
